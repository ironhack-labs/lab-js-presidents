...
function getNamesAndParty(presidentsArr) {
  let nameAndParty = presidentsArr.map((president) => {
    let presBasic = {
      name: president.name,
...
function getDemocraticPresidents(presidentsArr) {
  let demPresidents = presidentsArr.filter((president) => {
    return president.party === "Democratic"  
  })
  return demPresidents
}
...
function getAffiliatedPresidents(presidentsArr) {
  let affiliated = presidentsArr.filter((president) => {
    //the following returns any president object whose party property is 'truthy'
...
function  countYearsInOffice(presidentsArr) {
  let totalYears = presidentsArr.reduce((acc, cur) => {
    return acc + (cur.leftOffice ? (cur.leftOffice - cur.tookOffice): 0)
  }, 0)
  return totalYears
}
...
function countRepublicanPresidents(presidentsArr) {
  let numOfReps = presidentsArr.reduce((acc, curr) => {
    return acc + (curr.party === "Republican" ? 1 : 0)
  }, 0)
  return numOfReps
}
...
// should call the 'sort()' method on the presidents array passed as argument
// should return an array
// should return an array of presidents sorted by birth year, from oldest to youngest
...
// should use the 'sort()' method to sort the presidents array
// should return an array of objects
// should return an array of objects sorted alphabetically by the 'name' property