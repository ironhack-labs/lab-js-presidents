...
// Iteration 2: Name and Party  - `map()` TBC
...
function getDemocraticPresidents(presidentsArr) {
  const democraticPresidents = presidentsArr.filter((president) => {
    if (president.party === "Democratic") return president.name;
  });
...
function sortPresidentsByBirthYear(presidentsArr) {
  const sortedByAgePresidents = presidentsArr.sort((a, b) => {
    return a.birthYear - b.birthYear;
  });
  return sortedByAgePresidents;
}
