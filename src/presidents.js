...
function getNamesAndParty(presidentsArr) {
	return presidentsArr.map((presidentsCredential) => {
		return {
			name: presidentsCredential.name,
			party: presidentsCredential.party,
		};
	});
}
...
function getDemocraticPresidents(presidentsArr) {
	return presidentsArr.filter((ethnic) => ethnic.party === 'Democratic');
}
...
function getAffiliatedPresidents(presidentsArr) {
	return presidentsArr.filter((affiliated) => affiliated.party !== null);
}
...
function countYearsInOffice(presidentsArr) {
	return presidentsArr.reduce((acc, years) => {
		if (years.leftOffice === null) {
			return acc;
		} else {
			return acc + (years.leftOffice - years.tookOffice);
		}
	}, 0);
}
...
function countRepublicanPresidents(presidentsArr) {
	// return presidentsArr.reduce((acc, ethnicParty) => {
	// 	console.log(ethnicParty.party, acc);
	// 	if (ethnicParty.party === 'Republican') {
	// 		return acc + 1;
	// 	}
	// 	return acc;
	// }, 0);
	...
	return presidentsArr.reduce(
		(acc, ethnicParty) => (ethnicParty.party === 'Republican' ? acc + 1 : acc),
		0
	);
}
...
function getPresidentsBornAfter(presidentsArr, year) {
	return presidentsArr.filter((bornAfter) => bornAfter.birthYear > year )
	// return presidentsArr.filter((bornAfter) => {
	// 	if (bornAfter.birthYear > year) return bornAfter;
	// });
}
