...
function getNames(presidentsArr) {
  const names = presidentsArr.map(function(element){
    return element.name
  })
  return names
}
...
function getNamesAndParty(presidentsArr) {
  const namesAndParty = presidentsArr.map(function(element){
    return {
      name: element.name,
      party: element.party
    };
...
function  countYearsInOffice(presidentsArr) {
  const yearsInOffice = presidentsArr.reduce(function(acc, currentValue) {
...
function countRepublicanPresidents(presidentsArr) {
  const RepublicanPresidents = presidentsArr.reduce(function(acc, currentValue) {
...
function sortPresidentsByBirthYear(presidentsArr) {
  const presidentsByBirthYear = presidentsArr.sort((a,b) => a.birthYear - b.birthYear);
...
function getAgeAtInauguration(presidentsArr) {
  const AgeAtInauguration = presidentsArr.map(function(element){
    return {
...
function getPresidentsBornAfter(presidentsArr, year) {
  const presidentsBornAfter = presidentsArr.filter(function(element) {
    if (element.birthYear > year) { return element} 
  })
...
function sortPresidentsByName(presidentsArr) {
  const presidentsByName = presidentsArr.sort((a,b) => a.name.localeCompare(b.name))
