...
function getAffiliatedPresidents(presidentsArr) {
  presidentsArr.filter((president) => president.getAffiliatedPresidents);
}
...
function countYearsInOffice(presidentsArr) {
  const reduceArray = presidentsArr.reduce((acc, curred) => {
    return acc + (curred.leftOffice - curred.tookOffice);
  }, 0);
  return reduceArray;
}
