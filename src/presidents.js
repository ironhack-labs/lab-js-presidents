...
function getNamesAndParty(presidentsArr) {
  return presidentsArr.map((e) => ({ name: e.name, party: e.party }));
}
...
function countYearsInOffice(presidentsArr) {
  return presidentsArr.reduce(
    (acc, e) =>
      e.leftOffice !== null ? acc + e.leftOffice - e.tookOffice : acc,
    0
  );
  // let accVar = acc;
  // if (e.leftOffice !== null)  {
  //   accVar += e.leftOffice - e.tookOffice;
  // }
  // return accVar;
  // );
}
...
function sortPresidentsByBirthYear(presidentsArr) {
  return presidentsArr.sort((ce, ne) => ce.birthYear - ne.birthYear);
}
