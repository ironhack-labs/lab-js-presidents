...
function getNames(presidentsArr) {
  let preNames = presidentsArr.map((president, currentIndex, WholeArray) => {
...
function getNamesAndParty(presidentsArr) {
  let nameParty = presidentsArr.map((president) => {
...
function getDemocraticPresidents(presidentsArr) {
  let demPres = presidentsArr.filter((president) => {
    if (president.party === "Democratic") {
      return president.id, president.name, president.party;
    }
  });
  return demPres;
}
...
function getAffiliatedPresidents(presidentsArr) {
  let affiliated = presidentsArr.filter(
...
function countYearsInOffice(presidentsArr) {
  let officeTot = presidentsArr.reduce((acc, currentE) => {
    if (!currentE.leftOffice) {
      return acc;
    }
    return acc + currentE.leftOffice - currentE.tookOffice;
  }, 0);
