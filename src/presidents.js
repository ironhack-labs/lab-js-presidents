...
function getNames(presidentsArr) {
  return presidentsArr.map((presi) => presi.name);
}

// Iteration 2 | Democratic Presidents - `filter()`
function getDemocraticPresidents(presidentsArr) {
  return presidentsArr.filter((presi) => presi.party === "Democratic");
}
...
function getAgeAtInauguration(presidentsArr) {
  return presidentsArr.map((presi) => {
    presi.ageAtInauguration = presi.tookOffice - presi.birthYear;
    return presi;
  });
}
...
function getPresidentsBornAfter(presidentsArr, year) {
  return presidentsArr.filter((presi) => presi.birthYear > year);
}
