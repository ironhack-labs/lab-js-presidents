...
function  countYearsInOffice(presidentsArr) {
  const totalYears = presidentsArr.reduce((sumYears, president) => {
    //  if(president.leftOffice) {
    //   return (president.leftOffice - president.tookOffice) + sumYears
    // } else {
    //   return sumYears
    // } 
      return president.leftOffice ? (president.leftOffice - president.tookOffice) + sumYears : sumYears
  }, 0);
  return totalYears;
}
...
function getPresidentsBornAfter(presidentsArr, year) {
  const bornAfterArray = presidentsArr.filter((president) => {
    return president.birthYear > year;
  });
  return bornAfterArray;
}
...
function countRepublicanPresidents(presidentsArr) {
  const republicanPresidents = presidentsArr.reduce((counter, president) => {
    // if(president.party === 'Republican') {
    //   counter = counter + 1;
    // }
    // return counter;
    return president.party === 'Republican' ? counter + 1 : counter
  }, 0) 
...
function sortPresidentsByName(presidentsArr) {
  const sortPresidentsByName = presidentsArr.sort((a, b) => a.name - b.name);
  return sortPresidentsByName;
}
