...
function getNames(presidentsArr) {
  const namesArr = presidentsArr.map((Presidents) => presidents.name)
  return namesArr
}
...
function getNamesAndParty(presidentsArr) {
  const namesAndParty = presidentsArr.map((presidents) => presidents.namesAndParty)
  return namesAndParty
}


// console.log("getNamesAndParty(presidents)", getNamesAndParty(presidents));
console.log(getNameAndParty)



// Iteration 3: Democratic presidents - `filter()`
function getDemocraticPresidents(presidentsArr) {
  const getDemocraticPresidents = democratic.filter(eachPresidentsDemocratics => {
    if (eachPresidentsDemocratics presidents.party.democratic) {
      return true
    }
})
 }
