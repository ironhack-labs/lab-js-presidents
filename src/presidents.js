...
function getNamesAndParty(presidentsArr) {
  const presidentsPartyNames = presidentsArr.map(eachPresident => {
    return result = { name: eachPresident.name, party: eachPresident.party }
  })
  return presidentsPartyNames
}

// Iteration 3: Democratic presidents - `filter()`
function getDemocraticPresidents(presidentsArr) {
  const presidentDemocratics = presidentsArr.filter(eachPresident => {
    return eachPresident.party === "Democratic"
  })
  return presidentDemocratics
}


// Iteration 4: Affiliated Presidents - `filter()`
function getAffiliatedPresidents(presidentsArr) {
  const nonAfiliatedPresidents = presidentsArr.filter(eachPresident => {
    return eachPresident.party
  })
  return nonAfiliatedPresidents
}

// Iteration 5: Count Years in Office - `reduce()`
function countYearsInOffice(presidentsArr) {
  const presidentsYears = presidentsArr.reduce((acc, eachPresident) => {
