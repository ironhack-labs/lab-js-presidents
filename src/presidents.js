...
function countYearsInOffice(presidentsArr) {
  const totalYearsInOffice = presidentsArr.reduce((sum, president) => {
    return president.leftOffice !== null
      ? sum + (president.leftOffice - president.tookOffice)
      : sum;
  }, 0);
...
function countRepublicanPresidents(presidentsArr) {
  const presidentsCount = presidentsArr.reduce((sum, president) => {
    return president.party === "Republican" ? sum + 1 : sum;
  }, 0);
  return presidentsCount;
}
