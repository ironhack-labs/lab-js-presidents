...
function getDemocraticPresidents(presidentsArr) {
  return presidentsArr.filter(
    (currentPresident) => currentPresident.party === "Democratic"
  );
}

// Iteration 3 | Count Years in Office - reduce()
function countYearsInOffice(presidentsArr) {
  return presidentsArr.reduce((totalYears, currentPresident) => {
    if (currentPresident.leftOffice != null) {
      totalYears += currentPresident.leftOffice - currentPresident.tookOffice;
    }
    return totalYears;
  }, 0);
}
...
function getAgeAtInauguration(presidents) {
  return presidents.map((currentPresident) => {
    currentPresident.ageAtInauguration =
      currentPresident.tookOffice - currentPresident.birthYear;
...
function getPresidentsBornAfter(presidentsArr, year) {
  return presidentsArr.filter(
    (currentPresident) => currentPresident.birthYear > year
  );
}

// Bonus: Iteration 7 | Count Republican Presidents
function countRepublicanPresidents(presidentsArr) {
  return presidentsArr.reduce((amountOfRepublicans, currentPresident) => {
    if (currentPresident.party === "Republican") {
      amountOfRepublicans++;
    }
    return amountOfRepublicans;
  }, 0);
}
